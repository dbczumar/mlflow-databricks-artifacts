...
class TestDatabricksArtifactRepository(object):
    ...
    def test_init_artifact_uri(self, artifact_uri, expected_uri, expected_db_uri):
        with mock.patch(
            ...
            DATABRICKS_ARTIFACT_REPOSITORY_PACKAGE + ".get_tracking_uri", return_value="databricks://getTrackingUriDefault"
        ):
    ...
    def test_log_artifact_azure_with_headers(
        ...
        with mock.patch(
            ...
            DATABRICKS_ARTIFACT_REPOSITORY_PACKAGE + ".put_block"
        ) as mock_put_block, mock.patch(
            DATABRICKS_ARTIFACT_REPOSITORY_PACKAGE + ".put_block_list"
        ) as mock_put_block_list:
            mock_credentials = ArtifactCredentialInfo(
            ...
            mock_put_block.assert_called_with(MOCK_AZURE_SIGNED_URI, ANY, ANY, headers=expected_headers)
            mock_put_block_list.assert_called_with(MOCK_AZURE_SIGNED_URI, ANY, headers=expected_headers)
    ...
    def test_log_artifact_azure_blob_client_sas_error(self, databricks_artifact_repo, test_file):
        with mock.patch(
            ...
            write_credentials_mock.return_value = write_credentials_response_proto
            mock_put_block.side_effect = MlflowException("MOCK ERROR")
            with pytest.raises(MlflowException):
    ...
    def test_log_artifact_aws_with_headers(
        ...
        with mock.patch(
            DATABRICKS_ARTIFACT_REPOSITORY + "._get_write_credentials"
        ) as write_credentials_mock, mock.patch(DATABRICKS_ARTIFACT_REPOSITORY_PACKAGE + ".http_put") as request_mock:
            mock_credentials = ArtifactCredentialInfo(
    ...
    def test_databricks_download_file_get_request_fail(self, databricks_artifact_repo, test_file):
        with mock.patch(
            ...
            DATABRICKS_ARTIFACT_REPOSITORY_PACKAGE + ".http_get"
        ) as request_mock:
            mock_credentials = ArtifactCredentialInfo(
                signed_uri=MOCK_AZURE_SIGNED_URI, type=ArtifactCredentialType.AZURE_SAS_URI
            )
